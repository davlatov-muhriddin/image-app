...
const getImage = async (apiKey, img) => {
  const response = await fetch(`https://api.unsplash.com/search/photos/?client_id=${apiKey}&per_page=30&query=${img}`)
  return response.json()
}

if (!search.value) {
  getImage(API_KEY, "mauntains")
...
form.addEventListener("submit", (e) => {
  ...
  if (searchInputText == "") {
    ...
    getImage(API_KEY, searchInputText)
      .then((data) => showImage(data.results))
    form.reset()
  }
})
